...
MODEL_PATH = 'models/perros-gatos.h5'
...
def model_predict(img_path, model):
    ...
    img=cv2.resize(cv2.imread(img_path), (150, 150), interpolation = cv2.INTER_AREA)
    x=np.asarray(img)
...
def upload():
    if request.method == 'POST':
        ...
        print('PREDICCIÓN', names[np.argmax(pred)])
        
        # Enviamos el resultado de la predicción
        result = str(names[np.argmax(pred)])              
        return result
    return None
